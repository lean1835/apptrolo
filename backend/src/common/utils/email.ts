...
export const sendOTPEmail = async (toEmail: string, otp: string): Promise<boolean> => {
  ...
  if (!emailPass) {
    logger.warn(`[EMAIL BYPASS] EMAIL_PASS environment variable is not configured. Logged OTP to console: ${otp}`);
    return false;
  }
  ...
  try {
    ...
    logger.info(`✅ Email OTP successfully sent to ${toEmail}`);
    return true;
  } catch (error) {
    logger.error(`❌ Failed to send email OTP to ${toEmail}`, error);
    return false;
  }
};
